...
public static class GlobalEventManager
{
    ...
    public static void UnregisterHandler(string evt, GlobalEventHandler handler)
    {
        ...
        {
            immediateHandlersTable[evt] -= handler;
            if (immediateHandlersTable[evt] == null)
            {
    ...
    public static void SendEvent(GlobalEvent globalEvent)
    {
        ...
        {
            Delegate[] delegates = immediateHandlersTable[globalEvent.evtName].GetInvocationList();
            foreach (var del in delegates)
            {
                try
                {
                    if ((GlobalEventHandler)del != null)
                        ((GlobalEventHandler)del)();
                }
                catch (Exception e)
                {
                    Debug.LogError("Catch an exception when execute " + del.Target.ToString() + "." + del.Method.ToString());
