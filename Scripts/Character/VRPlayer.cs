...
public class VRPlayer : Singleton<VRPlayer>
{
    ...
    void Start()
    {
        Debug.LogWarning("VR Player Start");
    ...
    {
        if(LeftHand.gameObject.activeSelf==false)
        {
            LeftHand.gameObject.SetActive(true);
        }
        if (RightHand.gameObject.activeSelf == false)
        {
        ...
        if (CameraMask)
        {
            CameraMask.transform.parent = HeadCamera.transform;
    ...
    {
        SteamVR_TrackedObject headTrackedObject = VRHead.GetComponent<SteamVR_TrackedObject>();
        if (headTrackedObject == null)
            return false;

        return headTrackedObject.isValid;
    }
    ...
    #region Camera PostEffect Control
